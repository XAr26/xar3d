...
import {
  ...
  CheckCircle, Loader2, Heart, Share2, Box, CreditCard, Users
} from 'lucide-react';
...
export default function AssetDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    axios.get(`/api/v1/assets/${id}`)
      .then(res => {
        ...
        if (res.data.data.user) {
          setIsFollowing(res.data.data.user.is_following || false);
          setFollowerCount(res.data.data.user.follower_count || 0);
        }
      })
  ...
  const handleFollowToggle = async (e: React.MouseEvent) => {
    ...
    try {
      const res = await axios.post(`/api/v1/creators/${asset.user.id}/follow`);
      setIsFollowing(res.data.is_following);
      setFollowerCount(res.data.follower_count);
    } catch (e) {
      alert('Terjadi kesalahan saat mengikuti kreator.');
    }
  };
  ...
  return (
    <>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                ...
                <div className="flex items-center gap-4 text-sm text-brand-muted mb-6 flex-wrap">
                  <Link href={`/creators/${asset.user?.id}`} className="flex items-center gap-2 hover:text-white transition-colors bg-white/5 border border-white/10 px-3 py-1.5 rounded-xl">
                    <div className="w-5 h-5 rounded-full bg-gradient-to-tr from-brand-blue to-brand-purple flex items-center justify-center text-white text-[10px] font-bold">
                      {asset.user?.name?.[0]}
                    </div>
                    <div className="flex flex-col">
                      <span className="font-medium text-white">{asset.user?.name}</span>
                      <span className="flex items-center gap-1 text-[11px] text-purple-400 font-semibold mt-0.5">
                        <Users className="w-3 h-3" /> {followerCount} Pengikut
                      </span>
                    </div>
                  </Link>
                  <button 
                    onClick={handleFollowToggle}
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${isFollowing ? 'bg-white/10 text-white border-white/10' : 'bg-brand-blue text-white border-brand-blue hover:bg-brand-blue/90 shadow-lg shadow-brand-blue/20'}`}
                  >
      ...
      {isInsufficientBalanceModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            className="w-full max-w-md bg-brand-dark border border-white/10 rounded-3xl p-6 shadow-2xl text-center">
            ...
            <h3 className="text-xl font-bold text-white mb-2">Saldo Tidak Mencukupi</h3>
            <p className="text-brand-muted text-sm mb-6">
              Saldo akun Anda saat ini tidak cukup untuk membeli aset ini. Silakan isi saldo Anda atau cari aset yang gratis.
            </p>

            <div className="space-y-3">
              <Link href="/dashboard" className="w-full py-3.5 bg-brand-blue hover:bg-brand-blue-hover text-white rounded-xl font-semibold transition-all flex items-center justify-center gap-2">
                Lihat & Isi Saldo
              </Link>
              <Link href="/explore" className="w-full py-3.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-xl font-semibold transition-all flex items-center justify-center gap-2">
                Cari Aset Gratis
              </Link>
            </div>

            <button onClick={() => setIsInsufficientBalanceModalOpen(false)}
              className="w-full py-3 mt-4 text-brand-muted hover:text-white text-sm font-medium transition-colors">
              Tutup
            </button>
          </motion.div>
        </div>
