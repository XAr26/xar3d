...
import { Package, Globe, Star, Download, ArrowLeft, ExternalLink, UserCircle, Sparkles, BadgeCheck, Ban, Info, X, Users } from 'lucide-react';
...
export default function CreatorProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/v1/creators/${id}`)
      .then(res => {
        ...
        setFollowerCount(res.data.data.creator.follower_count || 0);
        setIsFollowing(res.data.data.creator.is_following || false);
      })
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      const res = await axios.post(`/api/v1/creators/${creator.id}/follow`);
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-6 items-center md:items-start">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <div className="flex items-center gap-4 justify-center md:justify-start mb-4">
                  <span className="flex items-center gap-1.5 text-sm text-purple-400 font-semibold bg-purple-500/10 px-3 py-1 rounded-lg border border-purple-500/20">
                    <Users className="w-4 h-4" /> {followerCount} Pengikut
                  </span>
                  <button 
                    onClick={handleFollowToggle}
                    className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all border ${isFollowing ? 'bg-white/10 text-white border-white/10 hover:bg-white/20' : 'bg-brand-blue text-white border-brand-blue hover:bg-brand-blue/90 shadow-lg shadow-brand-blue/20'}`}
                  >
